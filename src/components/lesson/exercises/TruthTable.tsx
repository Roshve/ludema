        ...
        <thead>
          <tr>
            {table.vars.map((v) => (
              <th
                key={v}
                className="rounded-lg bg-slate-100 px-3 py-2 text-base font-black text-slate-600 dark:bg-slate-800 dark:text-slate-300"
              >
                {v}
              </th>
            ))}
            <th className="rounded-lg bg-slate-800 px-3 py-2 text-base font-black text-white dark:bg-slate-950">
              {exercise.formula}
            </th>
          </tr>
        </thead>
          ...
          {table.rows.map((row, i) => (
            <tr key={i}>
              {table.vars.map((v) => (
                <td
                  key={v}
                  className="rounded-lg bg-slate-50 px-3 py-2 text-base font-extrabold text-slate-500 dark:bg-slate-800/60 dark:text-slate-400"
                >
                  {vstr(row.env[v])}
                </td>
              ))}
